...
float threeByteToVoltage(byte DB1, byte DB2, byte DB3)
{
  ...
  if (decimal <= 131071)
  {
    voltage = decimal*DAC_FULL_SCALE/131071;
  }
  else
  {
    voltage = -(262144-decimal)*DAC_FULL_SCALE/131072;
  }
  return voltage;
}
...
int voltageToDecimal(float voltage, byte *DB1, byte *DB2, byte *DB3)
{
  ...
  if (voltage >= 0)
  {
    decimal = voltage*131071/DAC_FULL_SCALE;
  }
  else
  {
    decimal = voltage*131072/DAC_FULL_SCALE + 262144;
  }
